...
class TimeFreqEncoder(nn.Module):
    ...
    def forward(self,x):
        lastrep,time_feature,cls=self.pretrained_model_time(x)  # [128, 110, 1024], [128, 1024], [128, 40]
        lstmcls,freq_feature=self.pretrained_model_freq(x)
        x = torch.cat((time_feature, freq_feature), dim=1)  # [128, 1152]

        lastrep = x
        encoded=x
        x = self.fc01(encoded)  # [128, 40]
