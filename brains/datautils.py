...
#Subject selecting
parser.add_argument('-sub','--subject', default=4 , type=int, help="choose a subject from 1 to 6, default is 0 (all subjects)")
...
class EEGDataset:
    ...
    def __init__(self, eeg_signals_path):
        ...
        if opt2.subject != 0:
            self.data = [loaded['dataset'][i] for i in range(len(loaded['dataset'])) if
                         loaded['dataset'][i]['subject'] == opt2.subject]
        else:
    ...
    def __getitem__(self, i):
        ...
        eeg = eeg[opt2.time_low:opt2.time_high, :]

        if opt2.model_type == "model10":
            eeg = eeg.t()
            eeg = eeg.view(1, 128, opt2.time_high - opt2.time_low)
...
def load_EEG(Path='data/EEG/'):
    ...
    # # Create loaders
    train_dataset = Splitter(dataset, split_path=opt2.splits_path, split_num=opt2.split_num, split_name="train")
    val_dataset = Splitter(dataset, split_path=opt2.splits_path, split_num=opt2.split_num, split_name="val")
    test_dataset = Splitter(dataset, split_path=opt2.splits_path, split_num=opt2.split_num, split_name="test")
