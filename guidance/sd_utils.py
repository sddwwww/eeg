...
from get_arg import opt


args2 = Namespace(
    save_path='exp/epilepsy/test',
    dataset='eeg',
    data_path='data/EEG/',
    device='cuda',
...
timefreq_model = TimeFreqEncoder(time_model, freq_model, args2)
...
eegtoclip_state_dict = torch.load(r'D:\aaa_code\BrainVis\exp\epilepsy\test\clipfinetune_model.pkl', map_location="cuda")
...
class StableDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def get_eeg_embeds(self, eeg_signals):  # eeg_signals:(1,59136)
        clip_encoded = model_eegtoclip(eeg_signals)  # (1,440,128)
        clip_encoded = clip_encoded.reshape(1, 77, 768)
        self.embeddings['pos'] = clip_encoded
        self.embeddings['neg'] = torch.zeros_like(clip_encoded)
        print("clip_encoded的形状：", clip_encoded.shape)
        print("clip_encoded 数据类型:", clip_encoded.dtype)
    ...
    def decode_latents(self, latents):
        latents = 1 / self.vae.config.scaling_factor * latents  # [1,4,64,64]

        imgs = self.vae.decode(latents).sample  # [1,3,512,512]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prompt", type=str, default="a delicious hamburger", help="Prompt for generating the image")
