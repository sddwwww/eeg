...
class MAEforEEG(nn.Module):
    ...
    def forward_encoder(self, x, mask_ratio):
        ...
        for blk in self.blocks:
            x = blk(x)
        x = self.norm(x)
        print(f"After LayerNorm: {x.shape}")  # 打印归一化后的形状, [1,33,1024]
...
if __name__ == '__main__':
    ...
    # # print(x)
    print("2：", x.shape)  # [1, 33, 1024]
    # # print(mask)
    # print(mask.shape)
    # # print(ids_restore)
    # print(ids_restore.shape)
    pred = mae.forward_decoder(x, ids_restore)  # [1, 128, 512]
